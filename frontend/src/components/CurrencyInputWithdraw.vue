<template>
  <div class="relative">
    <input
      ...
      :disabled="disabled"
      type="text"
      class="w-full border-0 text-xl font-bold bg-slate-100 ring-1 ring-slate-200 dark:bg-slate-800 dark:ring-slate-700 h-[50px] items-center justify-between rounded-[14px] py-0 pl-6 pr-40"
    ...
    <Menu v-if="!hideSelector" as="div" class="absolute right-0 inset-y-0 inline-block text-left">
      ...
      >
        ...
        <div v-if="selectedWithdrawToken" class="flex flex-nowrap gap-x-2 items-center pr-4">
          ...
          <div class="shrink-0 text-slate-500">{{ selectedWithdrawToken }}</div>
        </div>
...
const selectedWithdrawTokenImgSrc = computed(() => {
  return tokenImgSrc(props.selectedWithdrawToken);
});
</script>
